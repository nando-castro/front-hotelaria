...
interface ModalProps {
  isOpen: boolean;
  setIsOpen: () => void;
  dados: IQuarto | undefined;
}

// const { Option } = Select;

const EditarQuarto = ({ isOpen, setIsOpen, dados }: ModalProps) => {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      ...
      valor: dados?.valor,
      disponivel: dados?.disponivel === false ? "Não" : "Sim",
      vagas: dados?.vagas,
      foto: dados?.foto,
      categoria_quarto_id: dados?.categoria_quarto_id,
      dataCadastro: dayjs(dados?.dataCadastro, "YYYY-MM-DD"),
    });
  }, [
    dados?.categoria_quarto_id,
    dados?.dataCadastro,
    dados?.descricao,
    dados?.disponivel,
    dados?.foto,
